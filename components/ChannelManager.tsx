  ...
  return (
    <div className="grid gap-2 sm:grid-cols-2">
      {roles.map((role) => (
        <label
          key={role.id}
          className="flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-slate-700 dark:text-slate-200"
        >
          <input
            type="checkbox"
...
function ChannelCard({
  ...
  const [roleIds, setRoleIds] = useState(channel.allowed_role_ids);
  ...
  useEffect(() => {
    ...
    setRoleIds(channel.allowed_role_ids);
  }, [channel]);
    ...
    try {
      const body = await apiFetch<{ channel: AdminChannel }>(
        `/api/admin/channels/${channel.id}`,
        {
          ...
          body: JSON.stringify({ name, emoji, allowed_role_ids: roleIds })
        },
  ...
  return (
    ...
    >
      ...
      <div className="mt-3">
        <p className="mb-1.5 text-xs font-medium muted-text">Доступ для ролей</p>
        <RoleSelector
          roles={roles}
        ...
        {roleIds.length === 0 && (
          <p className="mt-1.5 text-xs text-amber-600 dark:text-amber-400">
            Ветка будет скрыта от всех ролей.
          </p>
        )}
      </div>
      ...
      <AnimatePresence>
        {confirmingDelete && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby={`delete-channel-${channel.id}`}
          >
            <motion.div
            ...
            >
              ...
              >
                Удалить ветку «{channel.name}»?
              </h4>
              <p className="mt-2 text-sm leading-6 muted-text">
                Все сообщения в этой ветке будут удалены без возможности
                восстановления.
              </p>
...
export default function ChannelManager({
  ...
  const loadChannels = useCallback(async () => {
    ...
    try {
      const body = await apiFetch<{ channels: AdminChannel[] }>(
        "/api/admin/channels",
        { cache: "no-store" },
  ...
  async function createChannel(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setChannels((current) =>
        [...current, body.channel].sort((left, right) =>
  ...
  return (
    <section className="panel overflow-hidden">
      <div className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-4">
        <div>
          ...
          <p className="mt-0.5 text-xs muted-text">
            Настройте название, эмодзи и доступ по ролям.
          </p>
        </div>
        <motion.button
          type="button"
      ...
      <div className="p-4">
        <AnimatePresence initial={false}>
          {showCreateForm && (
            <motion.form
              ...
              className="mb-4 rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4"
            ...
            >
              ...
              <div className="mt-3">
                <p className="mb-1.5 text-xs font-medium muted-text">
                  Доступ для ролей
                </p>
                <RoleSelector
                  roles={roles}
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                {submitting ? "Создаём…" : "Создать ветку"}
              </motion.button>
            </motion.form>
          )}
        </AnimatePresence>
        ...
        {loading ? (
          <p className="py-5 text-center text-sm muted-text">
            Загружаем ветки…
          </p>
        ) : (
          <div className="space-y-3">
            {channels.map((channel) => (
              <ChannelCard
                ...
                onDeleted={(channelId) =>
                  setChannels((current) =>
                    current.filter((item) => item.id !== channelId)
                  )
                }
